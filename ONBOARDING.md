...
| **learnings/** | Learnings catalog: `core.md` (**read first — 10 curated constitutional learnings**), `failure-patterns.md`, `process-rules.md`, `empirical-findings.md`, `architectural-rationale.md`, `domain-knowledge.md`. 59 graph-format entries (L-0001–L-0124, non-contiguous). | `core.md` on every fresh onboard — no exceptions. Type files when adding/reviewing learnings or during checkpoint step 4 active scan. |
...
See `learnings/core.md` for the curated essentials (read on every fresh onboard) and `learnings/` type-specific files for the full catalog (59 graph-format entries, L-0001–L-0124). All process lessons, failure modes, and session discipline rules are maintained there as the single source of truth.

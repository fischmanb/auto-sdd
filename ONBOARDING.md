...
| **learnings/** | Learnings catalog: `core.md` (**read first — 13 curated constitutional learnings**), `failure-patterns.md`, `process-rules.md`, `empirical-findings.md`, `architectural-rationale.md`, `domain-knowledge.md`. 77 graph-format entries (L-0001–L-00144, non-contiguous). | `core.md` on every fresh onboard — no exceptions. Type files when adding/reviewing learnings or during checkpoint step 4 active scan. |
...
See `learnings/core.md` for the curated essentials (read on every fresh onboard) and `learnings/` type-specific files for the full catalog (77 graph-format entries, L-0001–L-00144). All process lessons, failure modes, and session discipline rules are maintained there as the single source of truth.
...
4. **Learnings** → Default: something to capture (L-0116). Active scan: agent outcomes, corrections, new patterns, empirical findings, failures/near-misses, methodology signals for patterns (L-0124). Self-test new rules in same session (L-0117). Propagation check: core.md, counts, references (L-0114). Flag candidates for Brian — do NOT auto-write.
...
- Check for `.handoff.md` at repo root (ONLY on the very first prompt of a fresh chat — never mid-session). If it exists, read it for session-specific continuity from the retiring chat, absorb the context, then delete the file. See `.specs/HANDOFF-PROTOCOL.md`.

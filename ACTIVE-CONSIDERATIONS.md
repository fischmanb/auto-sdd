...
- **Learnings system — IMPLEMENTED (2026-03-01)**: 62 graph-compliant entries (L-0042–L-0103), ~47 old-format across other files. **core.md missing — #1 priority for next session** (referenced by ONBOARDING.md but never created — L-0090). Remaining: create core.md (~15-20 curated entries), old-format conversion (Prompts 4/5), index.md stale.
...
- **Learnings graph-schema conversion (2026-03-01)**: 62 graph-compliant entries (L-0042–L-0103) in agent-operations.md. ~47 old-format entries remain across design+general+performance+security files. Two agent prompts ready (Prompts 4 & 5) for converting those. Studio candidate.
...
- **Response scope discipline (2026-03-01)**: Two truncations in one session (L-0098, L-0103). Rule: count work items BEFORE first tool call, >3 items = split across responses. Checkpoint alone = ~12 tool calls. Not yet reliably internalized — watch for recurrence.
...
- **core.md creation — #1 NEXT SESSION PRIORITY (2026-03-01)**: ONBOARDING.md line 377 references core.md for fresh onboard but file doesn't exist. Fallback reads all of agent-operations.md (780+ lines). Need curated ~15-20 entries: highest-signal process rules and failure patterns. Top candidates: L-0063 (enumerate before acting), L-0074 (admin first), L-0078 (prescriptive state fix), L-0066 (rule gaming), L-0046/62/82 (evidence cluster), L-0042 (agent autonomy), L-0058 (agents never write learnings), L-0050 (HEAD sequencing), L-0092 (prompt stash), L-0097 (memory triggers/repo specifies), L-0098/L-0103 (scope discipline). Creating this file IS the handoff — without it, next session reads 780 lines or nothing.

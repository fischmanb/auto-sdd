...
- **Learnings system — IMPLEMENTED (2026-03-01)**: 39 graph-compliant entries (L-0042–L-0080), ~47 old-format across other files. core.md has 8 entries (old format). Remaining: old-format conversion (Prompts 4/5), back-references not fully bidirectional, index.md stale.
...
- **Learnings graph-schema conversion (2026-03-01)**: 39 graph-compliant entries (L-0042–L-0080) in agent-operations.md. ~47 old-format entries remain across design+general+performance+security files. Two agent prompts ready (Prompts 4 & 5) for converting those. Studio candidate. L-0076 establishes per-response learnings budget rule — may need ONBOARDING.md mention if it sticks.

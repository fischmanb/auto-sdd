...
- **Learnings graph-schema conversion (2026-03-01)**: ~112 entries across learnings files, 6 graph-compliant (L-0042–L-0047). Prompt 3 merged (L-0042/43/44). Deprecated notice on agent-operations.md fixed (old format deprecated, not file). Remaining: (1) agent-operations.md ~27 old-format entries, (2) design+general+performance+security ~57 entries. Two agent prompts ready (Prompts 4 & 5). Studio candidate.
- **ONBOARDING.md overhaul for efficient context (2026-03-01)**: Two parts. (1) Checkpoint expansion: Prompt 6 ready — expand checkpoint section so protocol scope visible inline without reading checkpoint.md. Prompt 1 branch lost, re-run needed. (2) Broader: audit ONBOARDING.md for context efficiency — does a fresh session get proper bootstrap without bloat? Current file may over-reference without inlining enough, or inline too much low-value content. Evaluate after checkpoint expansion lands.

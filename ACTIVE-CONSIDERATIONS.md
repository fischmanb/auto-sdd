...
- **Learnings system — IMPLEMENTED (2026-03-02)**: 77 graph-compliant entries (L-0001–L-00144, non-contiguous, mixed 4/5-digit IDs pending schema migration), 13 curated in `core.md`. Fresh onboard path works. Remaining: old-format conversion (Prompts 4/5), index.md stale, schema standardization branch merge pending.
...
- **Learnings graph-schema conversion (2026-03-02)**: 77 graph-compliant entries (L-0001–L-00144) across type-specific files. Some old-format entries may remain in `.specs/learnings/` files. Two agent prompts ready (Prompts 4 & 5) for converting those. Studio candidate.
- **Response scope discipline (2026-03-02)**: Formalized as L-00143 scope sizing system. Two truncations + one approval gate violation in one session (L-0098, L-0103, L-0104) led to systematic ritual: count items, estimate budget, check verification isolation, split or proceed. Agent prompt ready: `agent-prompt-engrain-L00143.md`. Pending: wire L-00143 into active infrastructure.
- **Wire L-00143 into active infrastructure**: Agent prompt ready (`agent-prompt-engrain-L00143.md`). Needs execution to embed scope sizing ritual into prompt templates, build loop entry points, and checkpoint protocol.
- **Build general-estimates.jsonl calibration logging**: L-00143 specifies continuous calibration from actuals for general system activities (not just build loop). Needs: logging harness for estimated vs actual tokens per activity type, mean error tracking, mandatory update trigger at >20% after 5+ runs.
- **HOW-I-WORK corpus curation (2026-03-02)**: Accumulation section has 90+ raw entries (including M-00072–M-00074) and zero curated sections. Full corpus read identified 4+ coherent clusters ready for formalization: (1) prompt engineering theory (~5 entries: token cost as quality metric, compression principle, boilerplate vs verbosity), (2) agent autonomy model (~4 entries: test→investigate→evaluate→verify→report, spec prescriptiveness calibration), (3) session type theory (~3 entries: sustained context for planning, fresh agents for execution, zero-code sessions as productive), (4) capture philosophy (~5 entries: capture-biased over precision-biased, density-matched, perishable lucidity). "We want to be 1" is the philosophical foundation connecting all clusters. Action: curate entries into named sections, promote mature patterns to learnings. L-0133 (corpus-level review) documents the method that surfaced this.
